                ...
                <div class="col-md-4 border border-left-0 border-light rounded offset-1 mx-2 bg-white">
                    ...
                    <div class="my-4 mx-2">
                        ...
                        <div class="row">
                            ...
                            <TaskViewRowInfo class='col-5' :params="formatDate(task.taskCreation)" />
                        </div>
                        <div class="row">
                    ...
                    <!-- Log hours modal -->
                        ...
                        <Teleport to="body">
                            <!-- use the modal component, pass in the prop -->
                            <FormPopUP :show="showEditTaskModal">
                                <template #header>
                                    <h3>Edit task</h3>
                                </template>
                                ...
                                <template #body>
                                    ...
                                    <input 
                                        ...
                                        v-model.trim="inputTask.title"
                                        >
                                    <label for="Title">Description</label>
                                    <textarea 
                                        class="form-control my-2"
                                        :placeholder="task.description"
                                        v-model.trim="inputTask.description"
                                        >
                                    </textarea>
                                    <label for="Title">Total hours</label>
                                    <input 
                                        type="number"
                                        class="form-control my-2"
                                        :placeholder="task.hoursAllocated"
                                        v-model.trim="inputTask.hoursAllocated"
                                        >
                                    <label for="Title">Start date</label>
                                    <input 
                                        type="date"
                                        class="form-control my-2"
                                        :placeholder="task.taskStartDate"
                                        v-model.trim="inputTask.taskStartDate"
                                        >
                                    <label for="Title">End date</label>
                                    <input 
                                        type="date"
                                        class="form-control my-2"
                                        :placeholder="task.taskEndDate"
                                        v-model.trim="inputTask.taskEndDate"
                                        >
                                </template>

                                <template #footer>
                                    <button class="btn btn-success" @click=acceptEditTask>Accept</button>
                                    <button class="btn btn-danger"  @click="showEditTaskModal=false">Cancel</button>
                                </template>
                            </FormPopUP>
                        </Teleport>
...
export default {
    ...
    setup(){
        ...
        const acceptEditTask = async () => {
            showEditTaskModal.value = false
            inputTask.value.filter(attribute=> attribute != '')
            console.log(inputTask.value)
            const params = {id: taskId, updatedTask: inputTask.value}
        ...
        onMounted(async () =>{
            ...
            // inputTask.value = await store.getters.getTask

        })

        return {
            task,showModal,input, AcceptLogHours, formatDate, showAsigneeModal,
            acceptNewAsignee, project, updateStatus, inputMember, errorHours,
            validateHours, inputTask, showEditTaskModal, acceptEditTask
        }
    }
